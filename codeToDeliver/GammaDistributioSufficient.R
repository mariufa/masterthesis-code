...
# Beta is the rate parameter, opposite of definition of gamma distr in report.
mleBeta = mleEstimators$estimate[2]
# Observed cramer-von mises
cramerObs = cramerVonMisesValueTest(gammaData, mleAlpha, mleBeta)


# Naive sampler. Uses one tolerance value for both sufficient statistics in naive sampling.
tolerance = 0.0001
...
hist(alg1sampCramer,breaks=200, main="", xlab="Cramer values", cex.lab=1.5)
abline(v = cramerObs, col="red")

hist(alg2sampCramer,breaks=200, main="", xlab="Cramer values", cex.lab=1.5)
abline(v = cramerObs, col="red")


# The next commented section is for multi-threading algorithm 1 and 2. P-values and cramer-von mises will not be saved in variables.
